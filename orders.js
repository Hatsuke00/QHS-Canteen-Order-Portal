(() => {
    ...
    const ORDER_EVENTS_CHANNEL = "qhs-order-events";
    const ORDER_EVENT_CREATED = "order-created";
    const ORDER_EVENT_SENDER_STUDENT = ROLE_STUDENT;
    const ORDER_EVENT_RECEIVER_STAFF = ROLE_STAFF;
    ...
    function bindStudentEvents(refs, state, user) {
        ...
        if (refs.checkoutBtn) {
            refs.checkoutBtn.addEventListener("click", () => {
                if (normalizeRole(user.role) !== ROLE_STUDENT) {
                    setInlineMessage(refs.orderMessage, "Only student accounts can submit orders.", "error");
                    return;
                }
                ...
                orders.push(order);
                saveOrders(orders);
                broadcastOrderPlaced(order, user.role);
    ...
    function initStaffDashboard(user) {
        const existingOrderIds = getOrders().map((order) => cleanText(order.orderId));
        const refs = {
            searchInput: document.getElementById("staff-orders-search"),
            filterSelect: document.getElementById("staff-orders-filter"),
            alertsBtn: document.getElementById("staff-enable-alerts-btn"),
            ordersEmpty: document.getElementById("orders-empty"),
            ordersList: document.getElementById("orders-list"),
            ordersCompletedEmpty: document.getElementById("orders-completed-empty"),
            ordersCompletedList: document.getElementById("orders-completed-list")
        ...
        const state = {
            knownOrderIds: new Set(existingOrderIds),
            notifiedOrderIds: new Set(existingOrderIds),
            pollTimerId: null,
            orderChannel: null
        };
        ...
        setupStaffOrderChannel(refs, state);
    ...
    function bindStaffEvents(refs, state, user) {
        ...
        if (refs.alertsBtn) {
            syncAlertsButtonState(refs.alertsBtn);
            refs.alertsBtn.addEventListener("click", () => {
                requestOrderAlertsPermission(refs.alertsBtn);
            });
        }
        ...
        window.addEventListener("storage", (event) => {
            ...
            syncStaffOrdersAndNotify(refs, state, getOrders());
        });
        ...
        window.addEventListener("beforeunload", () => {
            ...
            if (state.orderChannel) {
                state.orderChannel.close();
    ...
    function setupStaffOrderChannel(refs, state) {
        if (!("BroadcastChannel" in window)) {
            return;
        }

        try {
            state.orderChannel = new BroadcastChannel(ORDER_EVENTS_CHANNEL);
            state.orderChannel.addEventListener("message", (event) => {
                const payload = event.data;
                if (!isValidOrderEventPayload(payload)) {
                    return;
                }

                const orderId = cleanText(payload.order.orderId);
                if (!orderId || state.knownOrderIds.has(orderId)) {
                    return;
                }

                const latestOrders = getOrders();
                const matchedOrder = latestOrders.find((order) => cleanText(order.orderId) === orderId) || payload.order;
                announceNewOrdersToStaff([matchedOrder], state);
    ...
    function syncStaffOrdersAndNotify(refs, state, latestOrders) {
        const safeOrders = Array.isArray(latestOrders) ? latestOrders : [];
        ...
        announceNewOrdersToStaff(newOrders, state);
        state.knownOrderIds = new Set(safeOrders.map((order) => cleanText(order.orderId)));
        renderStaffOrders(refs);
    }

    function announceNewOrdersToStaff(orders, state) {
        ...
        const unseenOrders = orders.filter((order) => {
            const orderId = cleanText(order.orderId);
            if (!orderId || state.notifiedOrderIds.has(orderId)) {
                return false;
            }

            state.notifiedOrderIds.add(orderId);
            return true;
        });
        ...
        const latestOrder = unseenOrders[unseenOrders.length - 1];
        const label = unseenOrders.length === 1 ? "New order" : `${unseenOrders.length} new orders`;
        const studentName = cleanText(latestOrder.studentName) || "Student";
        const itemsText = summarizeOrderItemsForAlert(latestOrder.items);
        setSystemMessage(`${label}: ${studentName} ordered ${itemsText}.`, "success");
    }

    function summarizeOrderItemsForAlert(items) {
    ...
    function showBrowserOrderNotification(order) {
        ...
        const orderId = cleanText(order.orderId) || "New";
        const studentName = cleanText(order.studentName) || "Student";
        const summary = summarizeOrderItemsForAlert(order.items);
        const total = formatCurrency(order.totalPrice);

        try {
            const notice = new Notification(`Order ${orderId}`, {
                body: `${studentName}: ${summary} | ${total}`,
                tag: `order-${orderId}`
            });
            ...
            // Ignore notification delivery failures and keep in-app alerts.
        }
    }

    function requestOrderAlertsPermission(button) {
        if (!("Notification" in window)) {
            setSystemMessage("This browser does not support notifications.", "error");
            syncAlertsButtonState(button);
            return;
        }
        ...
        const handlePermission = (permission) => {
            ...
            if (permission === "granted") {
                setSystemMessage("Browser alerts enabled for incoming student orders.", "success");
                return;
            }

            if (permission === "denied") {
                setSystemMessage("Browser alerts are blocked. Allow notifications in browser settings.", "error");
                return;
            }

            setSystemMessage("Notification permission request was dismissed.", "error");
        };

        try {
            const requestResult = Notification.requestPermission();
            if (requestResult && typeof requestResult.then === "function") {
                requestResult.then(handlePermission);
                return;
            }

            handlePermission(Notification.permission);
        } catch (error) {
            setSystemMessage("Unable to request notification permission.", "error");
            syncAlertsButtonState(button);
        }
    }

    function syncAlertsButtonState(button) {
        if (!button) {
            return;
        }
    ...
    function broadcastOrderPlaced(order, senderRole) {
        if (!order || !("BroadcastChannel" in window)) {
            return;
        }

        if (normalizeRole(senderRole) !== ORDER_EVENT_SENDER_STUDENT) {
            return;
        }

        const payload = {
            type: ORDER_EVENT_CREATED,
            senderRole: ORDER_EVENT_SENDER_STUDENT,
            receiverRole: ORDER_EVENT_RECEIVER_STAFF,
            order: sanitizeOrderForBroadcast(order)
        };

        try {
            const channel = new BroadcastChannel(ORDER_EVENTS_CHANNEL);
            channel.postMessage(payload);
            channel.close();
        } catch (error) {
            // Ignore broadcast failures; localStorage sync still works as fallback.
        }
    }

    function sanitizeOrderForBroadcast(order) {
        return {
            orderId: cleanText(order.orderId),
            studentName: cleanText(order.studentName),
    ...
    function isValidOrderEventPayload(payload) {
        if (!payload || payload.type !== ORDER_EVENT_CREATED || !payload.order) {
            return false;
        }

        if (normalizeRole(payload.senderRole) !== ORDER_EVENT_SENDER_STUDENT) {
            return false;
        }

        if (normalizeRole(payload.receiverRole) !== ORDER_EVENT_RECEIVER_STAFF) {
            return false;
        }

        return Boolean(cleanText(payload.order.orderId));
    }
