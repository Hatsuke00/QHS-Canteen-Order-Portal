// QHS Canteen Portal
// Role-based Student/Staff canteen system with local storage + shared server sync.
...
const I18N = {
    en: {
        ...
        password_offline_unverified: "Current password could not be verified offline. Please connect to server.",
        ...
        delete_offline_unverified: "Current password could not be verified offline. Please connect to server.",
    ...
    fil: {
        ...
        password_offline_unverified: "Hindi ma-verify ang kasalukuyang password offline. Kumonekta sa server.",
        ...
        delete_offline_unverified: "Hindi ma-verify ang kasalukuyang password offline. Kumonekta sa server.",
...
const ApiService = {
    ...
    async request(payload) {
        if (!this.isEnabled()) return { ok: false, message: "API disabled." };
...
const StorageService = {
    ...
    getSession() {
        return this.read(STORAGE_KEYS.session, null);
    },
...
const ValidationService = {
    ...
    validateMenuItem(payload) {
        ...
        if (payload.imageUrl && !Helpers.safeImageUrl(payload.imageUrl)) return "Image URL must be a valid http/https URL.";
...
const AuthService = {
    ...
    shouldFallbackToLocalAuth(apiResult) {
        ...
        const message = String(apiResult?.message || "").toLowerCase();
        if (!message) return true;
        if (message.includes("network error")) return true;
        if (message.includes("database error")) return true;
        if (message.includes("server error")) return true;
        if (message.includes("invalid server response")) return true;
        ...
        if (message.includes("http 502")) return true;
        if (message.includes("http 503")) return true;
        if (message.includes("http 504")) return true;
        if (message.includes("api disabled")) return true;
        return false;
    },
    ...
    loginLocal(normalizedPayload) {
        ...
        if (Helpers.normalizeRole(matchedUser.role) === ROLES.student && SuspensionService.isSuspended(matchedUser.id)) {
            return { ok: false, message: "Your account is suspended. Please contact staff." };
        }
    ...
    changePasswordLocal(payload) {
        ...
        if (!currentStoredPassword) {
            return { ok: false, message: i18n("password_offline_unverified", "Current password could not be verified offline. Please connect to server.") };
        }
        ...
        if (!currentStoredPassword) {
            return {
                ...
                message: i18n(
                    ...
                    "Current password could not be verified offline. Please connect to server."
                )
            };
        }
        if (currentStoredPassword !== payload.currentPassword) {
...
const SuspensionService = {
    normalizeEntry(rawEntry) {
        ...
        return {
            ...
            suspendedAt: String(rawEntry?.suspendedAt || "").trim(),
    ...
    suspendStudent(userId, reason, actorUserId = "") {
        const targetId = String(userId || "").trim();
        ...
        const nextEntry = this.normalizeEntry({
            ...
            isSuspended: true,
            reason: nextReason,
            suspendedAt: previous?.suspendedAt || now,
            liftedAt: "",
            updatedAt: now,
...
const UIController = {
    ...
    async pullRemoteUpdates() {
        ...
        try {
            ...
            const hasRemoteChanges = menuChanged || ordersChanged || usersChanged || suspensionsChanged;
            ...
            if (
                ...
                this.showAuthView();
                window.location.hash = "";
                Helpers.setMessage(elements.authMessage, "Your account is suspended. Please contact staff.", "error", { toast: false });
                return;
            }
    ...
    bindAppEvents() {
        ...
        elements.suspensionsList?.addEventListener("click", (event) => {
            ...
            this.handleSuspensionAction(userId, action, reason);
        });
    ...
    setMenuImageZoom(nextScale) {
        ...
        if (elements.menuImagePreview) {
            elements.menuImagePreview.style.transform = `scale(${clamped})`;
    ...
    closeMenuImageModal() {
        ...
        if (elements.menuImagePreview) {
            elements.menuImagePreview.style.transform = "scale(1)";
        ...
        return new Promise((resolve) => {
            ...
            saveBtn?.addEventListener("click", () => {
                ...
                if (imageUrl && !Helpers.safeImageUrl(imageUrl)) {
                    Helpers.setMessage(elements.itemInfoModalMessage, "Image URL must be a valid http/https URL.", "error");
                    return;
                }
                if (!category) {
    ...
    handleSuspensionAction(userId, action, reason = "") {
        if (!RoleGuard.isStaff()) {
    ...
    renderSuspensions() {
        ...
        elements.suspensionsEmpty.classList.add("hidden");
        elements.suspensionsList.innerHTML = students
            .map((student) => {
                ...
                const inputReason = isSuspended ? reasonText : "";
                const suspendedAtText = String(entry?.suspendedAt || "").trim();
                const statusMeta = isSuspended && suspendedAtText
